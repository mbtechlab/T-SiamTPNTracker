...
from lib.train.actors import build_actor_st
...
def run_training(config_name, cudnn_benchmark=True, local_rank=-1, save_dir=None, base_seed=None,
    ...
    settings.description = 'Training script for T-SiamTPN stage1, and T-SiamTPN stage2'
    settings.config_name = config_name
    settings.project_path = 'train/{}'.format(config_name)
    if config_name_prv is not None:
        settings.project_path_prv = 'train/{}'.format(config_name_prv)
    ...
    config_module = importlib.import_module("lib.config.%s" % settings.config_name)
    cfg = config_module.cfg
    config_module.update_config_from_file(cfg, settings.cfg_file)
    ...
    if settings.config_name == "tpn_st":
        actor = build_actor_st(cfg, net)
    else:
