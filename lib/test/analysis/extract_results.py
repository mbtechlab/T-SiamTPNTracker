...
# Since extract_results is defined in this file, no need to import it
...
def calc_seq_err_robust(pred_bb, anno_bb, dataset, target_visible=None):
    ...
    # Adding print statement to check dataset value
    ...
    if torch.isnan(anno_bb).any():
        if dataset.lower() in ['uav', 'uav20l', 'uav123_10fps', 'uavtrack']:
            pass
        else:
            raise Exception('Warning: NaNs in annotation')

    # Handle frames with zero dimensions
    ...
    # Match prediction and ground truth lengths
    ...
    # Set first frame to ground truth
    pred_bb[0, :] = anno_bb[0, :]
    ...
    # Calculate center and overlap errors
    ...
    # Handle invalid cases based on dataset
    if dataset.lower() in ['uav', 'uav20l', 'uav123_10fps', 'uavtrack' , 'visdrone']:
...
def extract_results(trackers, dataset, report_name, skip_missing_seq=False, plot_bin_gap=0.05,
    ...
    with open(os.path.join(result_plot_path, 'eval_data.pkl'), 'wb') as fh:
