...
import lib.models.siamtpn_st.backbone.shufflenet_v2 as shuffle_module
from lib.models.siamtpn_st.util import FrozenBatchNorm2d
...
def build_backbone(cfg):
    # Using dictionary key access instead of attribute access
    if 'shufflenet' in cfg['MODEL']['BACKBONE']['TYPE']:
        model = Backbone_shufflenet(
            model_size=cfg['MODEL']['BACKBONE']['MODEL_SIZE'],
            out_stages=cfg['MODEL']['BACKBONE']['OUTPUT_STAGES'],
            freeze_bn=cfg['TRAIN']['FREEZE_BACKBONE_BN'], 
            frozen_layers=cfg['TRAIN']['FREEZE_LAYERS']
        )
    return model
