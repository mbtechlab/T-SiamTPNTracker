...
from lib.train.data import sampler, opencv_loader, processing, LTRLoader, jpeg4py_loader
...
def update_settings(settings, cfg):
    ...
    settings.num = cfg.MODEL.ANCHOR.NUM
    settings.factor = cfg.MODEL.ANCHOR.FACTOR
    settings.bias = cfg.MODEL.ANCHOR.BIAS
...
def build_dataloaders(cfg, settings):
    ...
    data_processing_train = processing.SiamTPNProcessing(search_area_factor=search_area_factor,
                                                       output_sz=output_sz,
    ...
    train_cls_score = getattr(cfg.TRAIN, "TRAIN_CLS_SCORE", False)
    ...
    dataset_train = sampler.TrackingSampler(datasets=names2datasets(cfg.DATA.TRAIN.DATASETS_NAME, settings, jpeg4py_loader),
                                            ...
                                            frame_sample_mode=sampler_mode, train_cls_score=train_cls_score)
