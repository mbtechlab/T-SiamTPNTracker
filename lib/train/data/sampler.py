...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
        ...
        self.train_cls_score = train_cls_score  # whether we are training classification score head
        ...
        if isinstance(max_gap, int):
            self.max_gap = max_gap
        elif isinstance(max_gap, list):
            if self.frame_sample_mode == 'causal':
                self.max_gap = max_gap[0]
            else:
                self.max_gap = max_gap
    ...
    def get_frame_ids_trident(self, visible):
        ...
        while None in template_frame_ids_extra or len(template_frame_ids_extra) < (self.num_template_frames - 1):
            template_frame_ids_extra = []
